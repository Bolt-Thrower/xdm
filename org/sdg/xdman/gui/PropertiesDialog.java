...
public class PropertiesDialog {
   ...
   static void showDetails(DownloadListItem item) {
      ...
      titleLbl.setFont(new Font("Dialog", 1, 14));
      ...
      if(item.cookies != null) {
         for(int i = 0; i < item.cookies.size(); ++i) {
            txt = txt + (String)item.cookies.get(i) + "\n";
         }
      }

      cookiesTxt.setText(txt);
      JOptionPane.showOptionDialog((Component)null, new Object[]{titleLbl, "\n" + getString("FILE_NAME"), fileTxt, getString("SAVED_IN"), folderTxt, getString("URL"), urlTxt, getString("REFERER"), refereTxt, getString("UA"), uaTxt, "\n" + getString("SIZE") + ": " + item.size, getString("TYPE") + ": " + item.type, getString("DATE") + ": " + item.lasttry, "\nCookies: ", jsp}, getString("PROP_TTL"), -1, 1, (Icon)null, (Object[])null, (Object)null);
   }
}
